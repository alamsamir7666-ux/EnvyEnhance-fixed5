...
// ??? E-commerce Events ????????????????????????????????????????????????????????
export function trackAddToCart(product: { id: number; name: string; price: number; category: string }) {
...
export function trackViewProduct(product: { id: number; name: string; price: number; category: string }) {
