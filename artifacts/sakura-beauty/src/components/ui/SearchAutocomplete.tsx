    ...
    <div className="relative w-full">
      <form onSubmit={handleSubmit}>
        <div className="relative flex items-center">
          ...
          {!loading && query && (
            <button type="button" onClick={() => { setQuery(""); setResults(null); setOpen(false); }} className="absolute right-3.5 text-muted-foreground hover:text-foreground">
              <X className="h-4 w-4" />
            </button>
          )}
        </div>
      ...
      {open && (
        <div
          className="absolute top-full mt-2 left-0 right-0 bg-card border border-border rounded-2xl shadow-lg z-50 overflow-hidden"
          onTouchStart={() => { suppressClose.current = true; }}
          onMouseDown={() => { suppressClose.current = true; }}
        >
          {!hasResults ? (
            ...
            <div className="py-2 max-h-[420px] overflow-y-auto">
              {results.categories.length > 0 && (
                <div>
                  ...
                  {results.categories.map((cat) => (
                    <button key={cat.slug}
                      onTouchEnd={() => goToCategory(cat.slug)}
                  ...
                  {results.products.map((product) => {
                    ...
                    return (
                      <button key={product.id}
                        onTouchEnd={() => goToProduct(product.id)}
              ...
              <div className="border-t mt-1">
                <button
                  onTouchEnd={() => { navigate(`/products?q=${encodeURIComponent(query.trim())}`); setOpen(false); onClose?.(); }}
                  onClick={() => { navigate(`/products?q=${encodeURIComponent(query.trim())}`); setOpen(false); onClose?.(); }}
