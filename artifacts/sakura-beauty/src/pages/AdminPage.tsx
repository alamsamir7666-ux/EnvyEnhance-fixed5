                        ...
                        <tr key={`${o.id}-expanded`} className="bg-pink-50/40">
                          <td colSpan={7} className="px-8 py-4">
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
                              ...
                              <div>
                                {(o.giftWrap === "true" || o.giftWrap === true) {o.giftWrap === "true" && ({o.giftWrap === "true" && ( (
...
function ReturnsTab() {
  ...
  async function updateStatus(id: number, status: string, adminNote?: string, refundAmount?: string) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setReturns(prev => prev.map(ret => ret.id === id ? updated : ret));
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {returns.length === 0 ? (
        <p className="text-sm text-muted-foreground py-8 text-center">No return requests yet.</p>
      ) : (
        <div className="space-y-3">
          {returns.map(ret => (
            <div key={ret.id} className="bg-card border rounded-xl p-5 space-y-3">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-medium text-sm">Return #{ret.id} — Order #{ret.orderId}</p>
                  ...
                  {ret.adminNote && <p className="text-xs text-blue-600 mt-1">Note: {ret.adminNote}</p>}
                </div>
                <span className={`text-xs px-2 py-1 rounded-full font-medium shrink-0 ${statusColors[ret.status] ?? "bg-muted"}`}>
                  {ret.status}
                </span>
              </div>
              {ret.status === "requested" && (
                <div className="flex gap-2 flex-wrap">
                  ...
                  <button onClick={() => { const note = prompt("Rejection reason?"); if(note) updateStatus(ret.id, "rejected", note); }}
                    disabled={updatingId === ret.id}
              ...
              {ret.status === "approved" && (
                <div className="flex gap-2 flex-wrap items-center">
                  <input
                    type="number" placeholder="Refund amount (৳)" min="0"
                    className="text-xs border rounded-full px-3 py-1.5 w-40 focus:outline-none focus:ring-1 focus:ring-accent"
                    onKeyDown={(e) => { if (e.key === "Enter") { updateStatus(ret.id, "completed", undefined, (e.target as HTMLInputElement).value); }}}
                  />
                  <span className="text-xs text-muted-foreground">Press Enter to complete</span>
                </div>
              )}
            </div>
          ))}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {(data.customerSegments ?? []).map((seg: any) => (
          <div key={seg.segment} className="bg-card border rounded-xl p-5 text-center">
            <p className="text-2xl font-bold text-accent">{seg.count}</p>
            <p className="text-sm font-medium mt-1">{seg.segment} Customers</p>
            <p className="text-xs text-muted-foreground mt-0.5">Avg spend ৳{seg.avgSpent?.toLocaleString()}</p>
          </div>
        ))}
      </div>
      ...
      {data.monthlyRevenue?.length > 0 && (
        <div className="bg-card border rounded-xl p-5">
          <h3 className="font-medium text-sm mb-4">Monthly Revenue (last 12 months)</h3>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={data.monthlyRevenue}>
              ...
              <Area type="monotone" dataKey="revenue" stroke="#f43f5e" fill="#fdf2f8" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {/* Top Products */}
      <div className="bg-card border rounded-xl p-5">
        <h3 className="font-medium text-sm mb-4">Top Products by Revenue</h3>
        <div className="space-y-3">
          {(data.topProductsByRevenue ?? []).slice(0, 8).map((p: any, i: number) => (
            <div key={p.id} className="flex items-center gap-3">
              <span className="text-xs text-muted-foreground w-5 text-right shrink-0">{i + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{p.name}</p>
                <p className="text-xs text-muted-foreground">{p.unitsSold} units · {p.category}</p>
              </div>
              <span className="text-sm font-semibold shrink-0">৳{Number(p.revenue).toLocaleString()}</span>
            </div>
          ))}
        </div>
      </div>
